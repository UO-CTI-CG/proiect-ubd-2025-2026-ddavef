import { useMemo } from 'react';

export const translations = {
  en: {
    brand: 'Oradea2Wheels',
    heroTitle: 'Rent bikes and scooters with live tracking',
    heroCopy: 'Book a ride, watch it move on the city map, and manage your trips effortlessly.',
    ctaBrowse: 'Browse vehicles',
    ctaAdmin: 'Go to admin',
    ctaBackHome: 'Back to home',
    mapTitle: 'Live map',
    mapSubtitle: 'Markers update as you select vehicles or create a rental.',
    available: 'Available',
    unavailable: 'Unavailable',
    rentNow: 'Rent now',
    renting: 'Renting...',
    loginTab: 'Login',
    registerTab: 'Create account',
    email: 'Email',
    password: 'Password',
    username: 'Username',
    fullName: 'Full name',
    login: 'Login',
    register: 'Register',
    logout: 'Logout',
    loginCta: 'Login',
    loggedInAs: 'Signed in as',
    rentSuccess: 'Rental created. Track it on the map.',
    rentRequiresLogin: 'Please sign in to rent a vehicle.',
    rentSimulating: 'Simulating ride...',
    loggingIn: 'Logging in...',
    registering: 'Creating account...',
    saving: 'Saving...',
    rentEnded: 'Ride finished: distance {distance} km, cost €{cost}.',
    vehiclesTitle: 'Pick your next ride',
    home: 'Home',
    rentalsNav: 'Rentals',
    adminTools: 'Admin tools',
    authRequired: 'Sign in to continue.',
    scooterArea: 'Scooter free-park area',
    bikeParking: 'Bike parking spots',
    adminTitle: 'Admin: add vehicles',
    adminDescription: 'Add scooters or bikes to the fleet.',
    adminVehiclesTitle: 'Fleet overview',
    adminUsersTitle: 'User accounts',
    refresh: 'Refresh',
    actions: 'Actions',
    deleteLabel: 'Delete',
    toggleAvailability: 'Set availability',
    noUsers: 'No users yet.',
    noVehicles: 'No vehicles yet.',
    vehicleName: 'Name',
    vehicleType: 'Type',
    vehiclePrice: 'Price per hour',
    vehicleDescription: 'Description',
    vehicleAvailable: 'Available',
    scooter: 'Scooter',
    bike: 'Bike',
    saveVehicle: 'Save vehicle',
    savedVehicle: 'Vehicle saved.',
    saveFailed: 'Could not save vehicle.',
    deleteFailed: 'Could not delete item.',
    updateFailed: 'Could not update item.',
    loadFailed: 'Could not load data.',
    noBikesParked: 'No bikes parked yet.',
    adminOnly: 'Admin only area',
    profileTitle: 'Profile',
    profileDescription: 'Update your account details.',
    profileDetails: 'Account details',
    updateProfile: 'Update profile',
    updatePassword: 'Update password',
    currentPassword: 'Current password',
    newPassword: 'New password',
    profileSaved: 'Profile updated.',
    passwordSaved: 'Password updated.',
    profileError: 'Update failed.',
    profile: 'Profile',
    adminProfileLocked: 'Admin profile cannot be changed.',
    theme: 'Theme',
    themeLight: 'Light',
    themeDark: 'Dark',
    themeAuto: 'Auto',
    homeLabel: 'Mobility made easy',
    homeUpsellTitle: 'Hop on in seconds—see bikes and scooters nearby',
    homeUpsellCopy: 'No clutter, just pick a vehicle and glide. Live map, instant start, one tap to rent.',
    homeUpsellCta: 'Open rentals map',
    rentalsLabel: 'Rentals',
    rentalsHeadline: 'Choose on the live map and start riding',
    rentalsSubhead: 'See availability, tap to rent, and watch your ride update in real time.',
    tileScooters: 'City scooters',
    tileScootersCopy: 'Zip through downtown with lightweight rides and instant unlocks.',
    tileBikes: 'Comfy bikes',
    tileBikesCopy: 'Sturdy frames, smooth gears, and racks for errands or campus runs.',
    tileNight: 'Night-ready',
    tileNightCopy: 'Built-in lights and reflective trims keep you visible after dark.',
  },
  ro: {
    brand: 'Oradea2Wheels',
    heroTitle: 'Inchiriaza biciclete si trotinete cu harti live',
    heroCopy: 'Rezerva, urmareste pe harta si gestioneaza cursele rapid.',
    ctaBrowse: 'Vezi vehiculele',
    ctaAdmin: 'Mergi la admin',
    ctaBackHome: 'Inapoi la pagina principala',
    mapTitle: 'Harta live',
    mapSubtitle: 'Marcajele se actualizeaza cand selectezi sau creezi o inchiriere.',
    available: 'Disponibil',
    unavailable: 'Indisponibil',
    rentNow: 'Inchiriaza acum',
    renting: 'Inchiriere...',
    loginTab: 'Autentificare',
    registerTab: 'Creeaza cont',
    email: 'Email',
    password: 'Parola',
    username: 'Utilizator',
    fullName: 'Nume complet',
    login: 'Autentificare',
    register: 'Inregistrare',
    logout: 'Deconectare',
    loginCta: 'Autentificare',
    loggedInAs: 'Autentificat ca',
    rentSuccess: 'Inchirierea a fost creata. Vezi pe harta.',
    rentRequiresLogin: 'Conecteaza-te pentru a inchiria un vehicul.',
    rentSimulating: 'Simulam cursa...',
    loggingIn: 'Autentificare...',
    registering: 'Creare cont...',
    rentEnded: 'Cursa s-a terminat: distanta {distance} km, cost €{cost}.',
    vehiclesTitle: 'Alege urmatoarea cursa',
    home: 'Acasa',
    rentalsNav: 'Inchirieri',
    adminTools: 'Unelte admin',
    authRequired: 'Conecteaza-te pentru a continua.',
    scooterArea: 'Zona libera trotinete',
    bikeParking: 'Parcari biciclete',
    adminTitle: 'Admin: adauga vehicule',
    adminDescription: 'Adauga trotinete sau biciclete in flota.',
    adminVehiclesTitle: 'Flota',
    adminUsersTitle: 'Conturi utilizatori',
    refresh: 'Reincarca',
    actions: 'Actiuni',
    deleteLabel: 'Sterge',
    toggleAvailability: 'Seteaza disponibilitatea',
    noUsers: 'Nu exista utilizatori.',
    noVehicles: 'Nu exista vehicule.',
    vehicleName: 'Nume',
    vehicleType: 'Tip',
    vehiclePrice: 'Pret pe ora',
    vehicleDescription: 'Descriere',
    vehicleAvailable: 'Disponibil',
    scooter: 'Trotineta',
    bike: 'Bicicleta',
      rentRequiresLogin: 'Conecteaza-te pentru a inchiria un vehicul.',
      rentSimulating: 'Simulam cursa...',
      loggingIn: 'Autentificare...',
      registering: 'Creare cont...',
      saving: 'Se salveaza...',
    saveFailed: 'Nu s-a putut salva vehiculul.',
    deleteFailed: 'Nu s-a putut sterge.',
    updateFailed: 'Nu s-a putut actualiza.',
    loadFailed: 'Nu s-au putut incarca datele.',
    noBikesParked: 'Nu sunt biciclete parcate.',
    adminOnly: 'Zona doar pentru admin',
    profileTitle: 'Profil',
    profileDescription: 'Actualizeaza detaliile contului.',
    profileDetails: 'Detalii cont',
    updateProfile: 'Actualizeaza profilul',
    updatePassword: 'Actualizeaza parola',
    currentPassword: 'Parola curenta',
    newPassword: 'Parola noua',
    profileSaved: 'Profil actualizat.',
    passwordSaved: 'Parola actualizata.',
    profileError: 'Actualizarea a esuat.',
    profile: 'Profil',
    adminProfileLocked: 'Profilul admin nu poate fi schimbat.',
    theme: 'Tema',
    themeLight: 'Luminoasa',
    themeDark: 'Intunecata',
    themeAuto: 'Auto',
    homeLabel: 'Mobilitate rapida',
    homeUpsellTitle: 'Urca in cateva secunde—vezi biciclete si trotinete aproape',
    homeUpsellCopy: 'Fara aglomerare: alege un vehicul si pleci. Harta live, start instant, un singur click.',
    homeUpsellCta: 'Deschide harta de inchirieri',
    rentalsLabel: 'Inchirieri',
    rentalsHeadline: 'Alege pe harta live si porneste cursa',
    rentalsSubhead: 'Vezi disponibilitatea, inchiriaza rapid si urmareste cursa in timp real.',
    tileScooters: 'Trotinete urbane',
    tileScootersCopy: 'Strecori rapid prin centru cu acces instant.',
    tileBikes: 'Biciclete confort',
    tileBikesCopy: 'Cadre solide, viteze line si portbagaj pentru drumuri zilnice.',
    tileNight: 'Gata de noapte',
    tileNightCopy: 'Lumini integrate si elemente reflectorizante pentru vizibilitate.',
  },
};

export function useTranslator(lang) {
  return useMemo(() => (key) => translations[lang]?.[key] ?? key, [lang]);
}
